...
func testSingleMeter(t *testing.T) {
	path := "test-1" // path is empty here due to the format string already introducing the slash
	...
	var readMeter types.Smartmeter
	err := json.NewDecoder(res.Body).Decode(&readMeter)
